...
public class ListingApiService : IListingService
{
    ...
    public async Task<SearchResultDto> SearchAsync(SearchFilterDto filters, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            sw.Stop();
            Console.WriteLine($"[ListingApiService] SearchAsync tamamlandÄ± â€” {sw.ElapsedMilliseconds}ms, totalCount={result.TotalCount}");
            return result;
        }
        catch (OperationCanceledException)
        {
            sw.Stop();
            Console.WriteLine($"[ListingApiService] SearchAsync iptal edildi â€” {sw.ElapsedMilliseconds}ms");
            throw;
        }
    }
