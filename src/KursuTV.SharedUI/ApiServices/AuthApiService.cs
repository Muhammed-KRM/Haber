...
public class AuthApiService : IAuthService
{
    ...
    private async Task<string> ExtractErrorMessageAsync(HttpResponseMessage response)
    {
        ...
        if (string.IsNullOrWhiteSpace(errorString)) return "Beklenmeyen API hatasÄ±.";
        ...
        catch { /* JSON deÄŸilse dÃ¼z string dÃ¶n */ }
    ...
    public async Task<AuthResultDto> RegisterAsync(UserRegisterDto dto)
    {
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                return result ?? new AuthResultDto { Success = false, ErrorMessage = "BaÄŸlantÄ± hatasÄ±." };
            }
            return new AuthResultDto { Success = false, ErrorMessage = await ExtractErrorMessageAsync(response) };
        }
        catch (Exception ex)
        {
            return new AuthResultDto { Success = false, ErrorMessage = $"AÄŸ HatasÄ±: {ex.Message}" };
        }
    }
    ...
    // Bu metot normalde back-end (Server) tarafÄ±nda Ã§aÄŸrÄ±lÄ±r, client API Ã¼zerinden bunu yapamaz
    public Task UpdateRefreshTokenAsync(Guid userId, string refreshToken, DateTime expiryTime)
    {
        throw new NotImplementedException("Bu metod Client API arayÃ¼zÃ¼nden tetiklenemez.");
    }
