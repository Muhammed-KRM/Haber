...
// Firebase Admin baÅŸlatma (google-services.json dosyasÄ± gerekir)
...
builder.Services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme)
    .AddJwtBearer(options =>
    {
        ...
        // SignalR iÃ§in token query string desteÄŸi
...
{
    c.SwaggerDoc("v1", new() { Title = "KursuTV API", Version = "v1" });
});

// === 4.1 RATE LIMITING (GÃ¼venlik) ===
...
builder.Services.AddCors(options =>
{
    ...
    options.AddPolicy("Production", b => b
        .WithOrigins(
            ...
            ?? ["https://kursutv.com", "https://www.kursutv.com"])
        .AllowAnyMethod()
        .AllowAnyHeader()
        .AllowCredentials());
});
...
// === MÄ°DDLEWARE PÄ°PELINE ===
app.UseMiddleware<RequestResponseLoggingMiddleware>();
app.UseMiddleware<ExceptionHandlingMiddleware>();

// Swagger her ortamda aÃ§Ä±k (API geliÅŸtirme kolaylÄ±ÄŸÄ± iÃ§in)
app.UseSwagger();
app.UseSwaggerUI(c =>
{
    c.SwaggerEndpoint("/swagger/v1/swagger.json", "KursuTV.API v1");
...
app.UseRateLimiter(); // Middleware kuyruÄŸunda Auth'dan sonra gelmesi uygun
app.MapControllers();
app.MapHub<KursuTV.API.Hubs.NotificationHub>("/hubs/notifications");
...
using (var scope = app.Services.CreateScope())
{
    ...
    // PostgreSQL'in hazÄ±r olmasÄ±nÄ± bekle (Docker race condition)
    var retries = 10;
    while (retries > 0)
    {
        try
        {
            ...
            logger.LogInformation("VeritabanÄ± migration baÅŸarÄ±lÄ±.");
            break;
        }
        catch (Exception ex)
        {
            retries--;
            if (retries == 0) throw;
            logger.LogWarning("VeritabanÄ±na baÄŸlanÄ±lamadÄ±, {Retries} deneme kaldÄ±. Hata: {Error}", retries, ex.Message);
    ...
    {
        context.Users.Add(new KursuTV.Data.Entities.User
        {
            Email = adminEmail,
