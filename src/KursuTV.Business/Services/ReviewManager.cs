...
public class ReviewManager : IReviewService
{
    ...
    public async Task<ReviewDto> CreateAsync(ReviewCreateDto dto, Guid reviewerId)
    {
        try
        {
            var listing = await _listingRepo.GetByIdAsync(dto.ListingId)
                ?? throw new NotFoundException("Ä°lan", dto.ListingId);

            if (listing.OwnerId == reviewerId)
                throw new BusinessException("Kendi ilanÄ±nÄ±za yorum yapamazsÄ±nÄ±z.");
            ...
            // Listing AverageRating ve ReviewCount gÃ¼ncelle
            ...
            // Reviewer bilgisiyle birlikte dÃ¶ndÃ¼r
    ...
    private static ReviewDto MapToDto(Review r) => new()
    {
        ...
        ReviewerName = r.Reviewer?.FullName ?? "KullanÄ±cÄ±",
